...
Click the **gear icon** (⚙) next to the filters to show or hide table columns. Choose which information to display:

| Column | Default | Description |
|--------|---------|-------------|
| Category | Visible | Item category with color dot |
| Quantity | Visible | Current stock level |
| Cost/Unit | Visible | Purchase cost per stock unit |
| Price/Unit HT | Hidden | Price per individual unit, ex. VAT (e.g., per can) |
| Price/Unit TTC | Hidden | Price per individual unit, inc. VAT |
| Units/Package | Hidden | Number of units per package (e.g., 12 cans/box) |
| Content/Unit | Hidden | Content per unit (e.g., 400g) |
| Supplier | Visible | Supplier name |
| Status | Visible | OK or Low Stock alert |
| Notes | Hidden | Item notes |
| Reorder Level | Hidden | Alert threshold |

Your column preferences are saved per restaurant and persist across sessions. The table scrolls horizontally when many columns are enabled.
