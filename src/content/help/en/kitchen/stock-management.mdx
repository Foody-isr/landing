...
**Stock Management** tracks your raw ingredient inventory — what's physically in your storage. It is separate from your menu: menu items are what customers order, stock items are what you use to make them.

Recipes bridge the two. When you link ingredients to menu items (in **Kitchen > Recipes**), the system can automatically deduct stock when daily operations are closed, calculate food cost, and predict what you need to order tomorrow.
...
| **Content per Unit** | No | What's inside each container — only shown for container units (unit, pack, box, bag) |
| **Content Unit** | No | Unit of the content (g, kg, ml, or l) |
| **Cost per Unit** | No | Your purchase cost per unit, used for inventory value and food cost calculations |
| **Reorder Threshold** | No | Quantity at which a low-stock alert triggers |
...
Instead of editing quantities directly, you record **transactions**. This creates an auditable history of every stock movement.
...
Save time by letting AI extract items from delivery notes and invoices automatically.

### Step 1: Upload

1. Click the **Import Delivery** button (sparkle icon) at the top of the page.
2. Upload a photo of a delivery note or a PDF invoice.
3. Click **Upload and Analyze**. The AI processes the document.

### Step 2: Review & Confirm

The AI extracts items, quantities, units, prices, and supplier name. For each item:

- **Match to existing stock** using the dropdown, or leave as "new" to create a new item.
- **Edit quantities and prices** — pack count, price per pack, and total price are editable and auto-calculate each other.
- **Assign a category** for new items.
- Review the final **cost per unit** calculation.

Click **Confirm Import** to save. Each item receives a "receive" transaction, and new items are created in your stock.

> **Tip:** Take clear, well-lit photos of delivery notes. The AI works best with legible text, consistent formatting, and visible prices. Blurry or partially obscured documents may need more manual corrections.
...
> **Tip:** Review your stock history regularly. Unexplained deductions or large variances between theoretical and actual usage can point to portioning issues, waste, or recipe inaccuracies.

> **Tip:** Combine stock management with **Daily Operations** for the full picture. Stock tracks what you have; daily operations tracks what you use, calculates food cost, and generates tomorrow's orders.
